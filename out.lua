...
-- =========================
-- Player Name -> Workspace Object Blue Highlight
-- =========================

local Players = game:GetService("Players")

local PLAYER_HIGHLIGHT_NAME = "DexHighlight_PlayerBlue"
...
local PLAYER_OUTLINE_TRANSPARENCY = 0

local function ensurePlayerHighlight(target)
	if not target or not target.Parent then return end

	-- prevent duplicates (our named highlight only)
	local existing = target:FindFirstChild(PLAYER_HIGHLIGHT_NAME)
	if existing and existing:IsA("Highlight") then
		-- ensure it stays correct (in case something changed it)
		existing.Adornee = target
		existing.FillColor = PLAYER_FILL_COLOR
		existing.FillTransparency = PLAYER_FILL_TRANSPARENCY
		existing.OutlineColor = PLAYER_OUTLINE_COLOR
		existing.OutlineTransparency = PLAYER_OUTLINE_TRANSPARENCY
		return
	end

	local h = Instance.new("Highlight")
	h.Name = PLAYER_HIGHLIGHT_NAME
	h.Adornee = target
	h.FillColor = PLAYER_FILL_COLOR
	h.FillTransparency = PLAYER_FILL_TRANSPARENCY
	h.OutlineColor = PLAYER_OUTLINE_COLOR
	h.OutlineTransparency = PLAYER_OUTLINE_TRANSPARENCY
	h.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop -- remove if you don't want through-walls
	h.Parent = target
end

local function findWorkspaceTargetsForPlayer(player)
	-- 1) Prefer the actual Character
	if player.Character and player.Character.Parent == workspace then
		return { player.Character }
	end

	-- 2) Fallback: workspace child named exactly like the player
	local obj = workspace:FindFirstChild(player.Name)
	if obj then
		return { obj }
	end
...
local function applyForPlayer(player)
	-- apply now (if already present)
	for _, target in ipairs(findWorkspaceTargetsForPlayer(player)) do
		ensurePlayerHighlight(target)
	end

	-- keep applying on respawn
	player.CharacterAdded:Connect(function(char)
		-- small delay helps if parts/parenting aren’t ready instantly
		task.wait(0.1)
		ensurePlayerHighlight(char)
	end)
end

-- Watch workspace for models/objects named after players appearing later
workspace.ChildAdded:Connect(function(child)
	local p = Players:FindFirstChild(child.Name)
	if p then
		-- If something named after the player appears, highlight it
		ensurePlayerHighlight(child)
	end
end)
...
Players.PlayerAdded:Connect(function(p)
	applyForPlayer(p)
	-- If their character already exists quickly, catch it
	task.defer(function()
		for _, target in ipairs(findWorkspaceTargetsForPlayer(p)) do
			ensurePlayerHighlight(target)
		end
	end)
end)
